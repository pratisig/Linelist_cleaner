...
def test_cli_help():
    result = subprocess.run([sys.executable, "-m", "linelist_cleaner.cli", "--help"], capture_output=True, text=True)
...
def test_cli_sample_and_inspect(tmp_path):
    ...
    res_sample = subprocess.run(
        [sys.executable, "-m", "linelist_cleaner.cli", "sample", "-t", "cholera", "-o", str(sample_file)],
        capture_output=True,
        text=True
    )
    ...
    res_inspect = subprocess.run(
        [sys.executable, "-m", "linelist_cleaner.cli", "inspect", str(sample_file)],
        capture_output=True,
        text=True
    )
    ...
    res_clean = subprocess.run(
        [sys.executable, "-m", "linelist_cleaner.cli", "clean", str(sample_file), "-o", str(cleaned_file)],
        capture_output=True,
        text=True
    )
